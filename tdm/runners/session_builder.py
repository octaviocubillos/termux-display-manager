...
def build_session_script(display_num: int, desktop_id: str, custom_command: Optional[str] = None, custom_list=None, backend: str = "termux-x11", dpi: int = 96) -> Path:
    ...
    if custom_command:
        ...
        if desktop_id == "kde":
            ...
            lines.append("export XDG_CURRENT_DESKTOP=KDE")
            lines.append("export DESKTOP_SESSION=plasma")
            lines.append("export KDE_SESSION_VERSION=6")
            lines.append("export QT_QPA_PLATFORM=xcb")
            lines.append("if command -v startplasma-x11 >/dev/null 2>&1; then")
            lines.append("    exec startplasma-x11")
            lines.append("elif command -v plasma_session >/dev/null 2>&1; then")
            lines.append("    exec plasma_session")
            lines.append("elif command -v plasma-session >/dev/null 2>&1; then")
            lines.append("    exec plasma-session")
            lines.append("else")
            lines.append("    kwin_x11 --replace &")
