#!/usr/bin/env bash
# ==============================================================================
# 📦 [TDM] Instalador específico para Openbox
# ==============================================================================
set -e
export DEBIAN_FRONTEND=noninteractive
PREFIX_PATH="${PREFIX:-/data/data/com.termux/files/usr}"

echo "[TDM_PROGRESS:10:Preparando paquetes de Openbox]"
echo "📦 [TDM] Instalando Openbox..."

SUDO=""
if [ "$(id -u)" -ne 0 ] && [ ! -d "/data/data/com.termux" ]; then
    if command -v sudo >/dev/null 2>&1; then SUDO="sudo"; elif command -v doas >/dev/null 2>&1; then SUDO="doas"; fi
fi

if command -v pkg >/dev/null 2>&1; then
    pkg install -y x11-repo || true
    pkg update -y || true
    echo "[TDM_PROGRESS:60:Instalando paquetes de Openbox, audio y batería]"
    pkg install -y openbox obconf-qt tint2 aterm pavucontrol pulseaudio || {
        for p in openbox obconf-qt tint2 aterm pavucontrol pulseaudio; do pkg install -y "$p" || true; done
    }
elif [ -f "/etc/alpine-release" ] || command -v apk >/dev/null 2>&1; then
    $SUDO apk update || true
    $SUDO apk add openbox tint2 xterm || true
elif command -v apt-get >/dev/null 2>&1; then
    $SUDO apt-get update -y || true
    $SUDO apt-get install -y openbox obconf tint2 xterm || true
elif command -v pacman >/dev/null 2>&1; then
    $SUDO pacman -Sy --noconfirm openbox tint2 xterm || true
elif command -v dnf >/dev/null 2>&1; then
    $SUDO dnf install -y openbox tint2 xterm || true
fi

echo "[TDM_PROGRESS:100:Openbox instalado con éxito]"
echo "✅ [TDM] Instalación de Openbox completada."
